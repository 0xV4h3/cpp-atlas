...
ClangCompiler::ClangCompiler(const QString& execPath, const QString& compilerId)
    : m_id(compilerId), m_execPath(execPath)
{
}

QString ClangCompiler::id() const {
    return m_id;
}

QString ClangCompiler::name() const {
    return QString("Clang %1").arg(version());
}

QString ClangCompiler::executablePath() const {
    return m_execPath;
}

QStringList ClangCompiler::supportedStandards() const {
    return {"c++11", "c++14", "c++17", "c++20", "c++23"};
}

QString ClangCompiler::defaultStandard() const {
    return "c++17";
}
...
QString ClangCompiler::version() const {
    if (!m_versionQueried) {
        QProcess process;
        process.start(m_execPath, QStringList() << "--version");
        process.waitForFinished(3000);
        ...
        // Parse version from first line: clang version 17.0.0
        QRegularExpression versionRe(R"(version\s+(\d+\.\d+\.\d+))");
...
QString ClangCompiler::optimizationFlag(OptimizationLevel level) const {
    switch (level) {
...
CompileResult ClangCompiler::compile(const CompileRequest& request) {
...
QList<DiagnosticMessage> ClangCompiler::parseDiagnostics(const QString& output) {
...
QProcess* ClangCompiler::runExecutable(const QString& exePath, const QStringList& args) {
